...
def sound_check(obj: Laser): # To see if the laser is out of bound or not
    if obj.y < 0 or obj.y > 750:
        ...
        return True
# Thêm biến game_over để theo dõi trạng thái game.
game_over = False
def main():
    run = True
    lost = False
    ...
    while run:
        ...
        if lives <= 0 or player.health <= 0:
            ...
            lost_count += 1
        if lost:
            if lost_count > FPS * 10:  # Adjust this line for the desired time delay (10 seconds in this example)
                run = False
            else:
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_b:
                            ...
                            main()
                            game_over = False
                            return  # Add this line to exit the current main() call
...
game_over = False
def main():
    global lives, game_over  # Add the 'global game_over' declaration
    lives = 10
    game_over = False
    run = True
    lost = False
    ...
    def redraw_window():
        ...
        if lost:
            ...
            play_again_label = main_font.render("Press 'L' to play again", 1, (255, 255, 255))
            WIN.blit(play_again_label, ((WIDTH - play_again_label.get_width()) // 2, (HEIGHT - lost_label.get_height()) // 2 + 50))
...
def display_instructions():
    ...
    while run_instructions:
        ...
        button_rect = pygame.Rect((WIDTH - button_label.get_width()) // 2, 400, button_label.get_width(),
                                  button_label.get_height())
